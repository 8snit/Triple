...
namespace Triple.Api.EntityFramework
{
    public class DbContextScope : IDbContextScope
    {
            ...
            IDbContextFactory dbContextFactory = null)
        {
            ...
            this._parentScope = GetAmbientScope();
            if (this._parentScope != null && joiningOption == DbContextScopeOption.JoinExisting)
            {
                if (this._parentScope._readOnly && !this._readOnly)
                {
                ...
                this._nested = true;
                this._dbContexts = this._parentScope._dbContexts;
            }
            else
            {
                this._nested = false;
                this._dbContexts = new DbContextCollection(readOnly, isolationLevel, dbContextFactory);
            }
        ...
        public void RefreshEntitiesInParentScope(IEnumerable entities)
        {
            ...
            foreach (IObjectContextAdapter contextInCurrentScope in this._dbContexts.InitializedDbContexts.Values)
            {
                var correspondingParentContext =
                    this._parentScope._dbContexts.InitializedDbContexts.Values.SingleOrDefault(
                ...
                {
                    continue;
                    // No DbContext of this type has been created in the parent scope yet. So no need to refresh anything for this DbContext type.
                }
                ...
                foreach (var toRefresh in entities)
                {
                        ...
                        out stateInCurrentScope))
                    {
                            ...
                            out stateInParentScope))
                        {
                            ...
                            {
                                await
                                    correspondingParentContext.ObjectContext.RefreshAsync(RefreshMode.StoreWins,
                                        stateInParentScope.Entity).ConfigureAwait(false);
                            }
                        }
                    }
                }
        ...
        public void Dispose()
        {
            ...
            if (this._parentScope != null)
            {
                if (this._parentScope._disposed)
                {
                    /*
...
" + Environment.StackTrace;
                ...
                {
                    SetAmbientScope(this._parentScope);
                }
            }
        ...
        {
            return this._dbContexts.Commit();
        }

        private Task<int> CommitInternalAsync(CancellationToken cancelToken)
        {
            return this._dbContexts.CommitAsync(cancelToken);
        }

        private void RollbackInternal()
        {
            this._dbContexts.Rollback();
        }
    }
